...
# 定义Lax-Friedrichs通量分裂方法
def lax_friedrichs_flux_split(rho, u, p):
    c = np.sqrt(gamma * p / rho)
    lam = np.array([[u - c, 0,0],
                   [0, u, 0],
                   [0, 0, u + c]]
    ...
    lam_max=max(u-c,u,u+c)
    A_plus=0.5*(A+lam_max*np.eye(3))
    A_minus=0.5*(A-lam_max*np.eye(3))
...
def solve_sod_shock_tube_SW(nx, L, dt, t_end):
    ...
    for n in range(nt):
        for x in range(nx-2):#
            ...
            r_x_positive,phi_x_positive=superBee(x,U,1)
            r_x_negtive,phi_x_negtive=superBee(x,U,-1)
            r_x_minus1,phi_x_minus1=superBee(x-1,U,-1)
            r_x_plus1,phi_x_plus1=superBee(x+1,U,1)
...
# 定义求解Sod激波管问题的主函数
def solve_sod_shock_tube_LF(nx, L, dt, t_end):
    dx = L / (nx - 1)
    # dt = t_end / (nt - 1)
    nt=int((t_end/dt)+1)
    CFL=dt/dx
    ...
    for n in range(nt):
        for x in range(nx-2):#
            ...
            if x ==0 :
                continue
            # 使用中心差分计算每个单元界面的通量
            A,A_plus, A_minus = lax_friedrichs_flux_split(rho[x], u[x], p[x])
...
# 求解Sod激波管
x_list, rho_LF,u_LF,p_LF = solve_sod_shock_tube_LF(nx=200, L=2.0, dt=1e-5, t_end=0.001)
...
plt.plot(x_list[:-2], rho_LF[:-2],linestyle='dashed', label='Density_LF',color='orange')
plt.plot(x_list[:-2], rho_SW[:-2], linestyle='dashdot',label='Density_SW',color='green')
plt.xlabel('Position')
plt.ylabel('Density')
plt.legend()
...
plt.plot(x_list[:-2], u_LF[:-2], label='Velocity_LF', linestyle='dashed',color='orange')
plt.plot(x_list[:-2], u_SW[:-2], linestyle='dashdot',label='Velocity_SW', color='green')
plt.xlabel('Position')
plt.ylabel('Velocity')
plt.legend()
...
plt.plot(x_list[:-2], p_LF[:-2], label='Pressure_LF',linestyle='dashed', color='orange')
plt.plot(x_list[:-2], p_SW[:-2],linestyle='dashdot', label='Pressure_SW', color='green')
plt.xlabel('Position')
plt.ylabel('Pressure')
plt.legend()
